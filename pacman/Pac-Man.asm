...
proc PacManEatingSound
   ; CX = number of notes
    mov cx, 10
    lea si, [pacnotes]        ; Point SI to the start of the notes
    lea di, [pacnotesduration]    ; Point DI to the start of the durations
@@NextNote:
    push cx                           ; Save CX (loop counter)
    mov     bx, [si]                  ; Load note frequency into BX
    mov     al, 10110110b             ; Command byte for timer
    out     43h, al                   ; Send command to port 43h
    mov     ax, bx                    ; Load BX into AX
    out     42h, al                   ; Send lower byte of AX to port 42h
    mov     al, ah                    ; Load upper byte of AX into AL
    out     42h, al                   ; Send upper byte of AX to port 42h
    in      al, 61h                   ; Read port 61h
    or      al, 00000011b             ; Set bits 0 and 1
    out     61h, al                   ; Write to port 61h to enable speaker
    mov     ax, [di]                  ; Load duration from DI into AX
    call ShortDelay                     ; Call delay procedure
    add     si, 2                     ; Move to the next note
    add     di, 2                     ; Move to the next duration
    pop     cx                        ; Restore CX
    loop    @@NextNote                  ; Loop for the next note
    in      al, 61h                   ; Read port 61h
    and     al, 11111100b             ; Clear bits 0 and 1
    out     61h, al                   ; Write to port 61h to disable speaker
    ret
endp PacManEatingSound
